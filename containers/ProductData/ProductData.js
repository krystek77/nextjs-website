...
function ProductData({ features, parameters, initialState = 1 }) {
  const [tab, setTab] = React.useState(initialState);
  const handleToggleTab = (tab) => setTab(tab);

  return (
    <section className={styles.productData} id="specyfikacja">
      <div className={styles.productData__buttons}>
        <Tab
          title="Cechy"
          toggleTab={() => handleToggleTab(0)}
          classes={tab === 0 ? 'tab_productData tab_active' : 'tab_productData'}
        />
        <Tab
          title="Specyfikacja"
          toggleTab={() => handleToggleTab(1)}
          classes={tab === 1 ? 'tab_productData tab_active' : 'tab_productData'}
        />
      </div>
      {tab === 0 && (
