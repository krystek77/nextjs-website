...
const reasons = ["oferta na wyposażenie pralni przemysłowej", "części zamienne", "wsparcie techniczne", "zlecenie serwisu", "gwarancja", "dokumnety urządzeń", "ogólne", "księgowość", "współpraca"];
const markets = [
  "pralnie komercyjne",
...
function ContactForm() {
  ...
  const validateForm = () => {
    ...
    if (formData.email.match(/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/) === null) {
    ...
    //not required
    if (formData.company !== "" && formData.company.match(/^[a-zA-ZąćęłńóśźżĄĘŁŃÓŚŹŻ\s\.-]{3,}$/) === null) {
        ...
        <div className={styles.contactFormSection__container}>
          <header className={styles.contactFormSection__header}>
            ...
            <Description classes='description_maxWidth_960 description_18'>
              Masz pytania dotyczące przemysłowych urządzeń pralniczych marki <strong>Pralma</strong> lub <strong>Primus</strong>? Aby uzyskać pomoc techniczną, rozwiązć problemy z eksploatcją, konserwacją urządzeń, zapytać o ofertę na odpowiednie do Twoich potrzeb wyposażenie pralnicze lub masz
              inne pytania dotyczące naszej współpracy lub dokumnetów, wypełnij i prześlij poniższy formularz.
            </Description>
          </header>
          <form className={styles.contactFormSection__form} onSubmit={handleForm}>
            <div className={styles.contactFormSection__reason}>
              <SelectInput options={reasons} action={handleInput} name='reason' selected={formData.reason} />
            </div>
            <div className={styles.contactFormSection__market}>
              <SelectInput options={markets} action={handleInput} name='market' selected={formData.market} />
            </div>
            <div className={styles.contactFormSection__name}>
              <InputError message='Imie ma tylko litery i minimum 3 znaki' isError={errors.name} />
              <Input type='text' fieldName='name' handleInput={handleInput} value={formData.name} placeholder='imię' />
            </div>
            <div className={styles.contactFormSection__lastName}>
              <InputError message='Nazwisko ma tylko litery i minimum 3 znaki' isError={errors.lastName} />
              <Input type='text' fieldName='lastName' handleInput={handleInput} value={formData.lastName} placeholder='nazwisko' />
            </div>
            <div className={styles.contactFormSection__phone}>
              <InputError message='Niepoprawny numer telefonu' isError={errors.phone} />
              <Input type='phone' fieldName='phone' handleInput={handleInput} value={formData.phone} placeholder='telefon np: +48413450561' />
              <RequiredMarker classes='requiredMarker_bottom_1_5' />
            </div>
            <div className={styles.contactFormSection__city}>
              <InputError message='Miasto ma tylko litery, spację i minimum 3 znaki' isError={errors.city} />
              <Input type='text' fieldName='city' handleInput={handleInput} value={formData.city} placeholder='miasto' />
            </div>
            <div className={styles.contactFormSection__company}>
              <InputError message='Firma ma tylko litery, spację i minimum 3 znaki' isError={errors.company} />
              <Input type='text' fieldName='company' handleInput={handleInput} value={formData.company} placeholder='firma / instytucja' />
            </div>
            <div className={styles.contactFormSection__email}>
              <InputError message='Nie poprawny adres email' isError={errors.email} />
              <Input type='text' fieldName='email' handleInput={handleInput} value={formData.email} placeholder='email' />
              <RequiredMarker classes='requiredMarker_bottom_1_5' />
            </div>
            <div className={styles.contactFormSection__message}>
              <InputError message='Wiadomość musi zawierać conajmniej 50 znaków ...' isError={errors.message} />
              <textarea rows='10' name='message' onChange={handleInput} value={formData.message} placeholder='wpisz treść...' />
              <RequiredMarker classes="requiredMarker_bottom_minus_1" />
            </div>
            ...
            <div className={styles.contactFormSection__actionButtons}>
              <Button classes='button_mr_2 button_no_wrap' label='wyślij wiadomość' type='submit' />
              <Button classes='button_dangerous' label='zresetuj' type='button' action={reset} />
            </div>
          </form>
        </div>
      </section>
