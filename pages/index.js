...
function Home(props) {
  return (
    <React.Fragment>
      <HeadMetaTags />
      <main>
        <ModernLaundryEquipment product_cards={props.product_cards} products_title={props.products_title} products_subtitle={props.products_subtitle} classes='modernLaundryEquipment_bg_primary' />
        <ModernLaundryEquipment product_cards={props.product_cards_vend} products_title={props.products_title_vend} products_subtitle={props.products_subtitle_vend} />
      </main>
      <HygieneBarrier />
...
export async function getStaticProps() {
  ...
  const product_cards_all = await categoriesCollection
    ...
    .toArray();
  const product_cards_vend = product_cards_all.filter((product_card) => product_card.vend === true);
  const product_cards = product_cards_all.filter((product_card) => product_card.vend === false);
  ...
  return {
    props: {
      ...
      page_title: "",
      page_label: "",
      products_title: "Najwyższa jakość i doskonałe rezultaty prania",
      products_subtitle: "Przemysłowe urządzenia pralnicze",
      product_cards: JSON.parse(JSON.stringify(product_cards)),
      products_title_vend: "Nowoczesne pralnie samoobsługowe",
      products_subtitle_vend: "Przemysłowe urządzenia pralnicze do samoobsługi",
