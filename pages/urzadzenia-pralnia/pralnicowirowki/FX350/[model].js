...
function FX350(props) {
  ...
  return (
    <React.Fragment>
      ...
      <main>
        <ProductInfo model={model} line={line} category={category} description={description} available_models={available_models} slider={slider} vertical={isSliderVertical} />
...
export async function getStaticPaths(context) {
  return {
    paths: [{ params: { model: "FX-350" } }, { params: { model: "FX-450" } }, { params: { model: "FX-600" } }],
    fallback: false,
  };
}
...
export async function getStaticProps(context) {
  const washerExtractor = products.find((item) => item.model === context.params.model);
  return {
